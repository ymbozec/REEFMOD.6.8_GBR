...
function priority_list = f_generate_priority_list(priority_option, MY_REEFS, CONNECT)
...
MY_REEFS.Reef_ID = MY_REEFS.Reef_ID;
ORDERS = nan(length(MY_REEFS.Reef_ID), 6);
...
if priority_option.link_strength == 0 ; ORDERS(:,3) = 1;
    ...
    self = 1 + diag(-1 * ones(1,length(MY_REEFS.Reef_ID))); % connectivity matrix of ones with a diagonal of zeros
...
if priority_option.reef_area == 0
    ...
    ORDERS(:,5) = randperm(length(MY_REEFS.Reef_ID));

else
    
    switch priority_option.reef_area
        
        case 1; [~,I] = sort(MY_REEFS.Reference_Area_km2(MY_REEFS.Reef_ID),'descend'); % largest area first
        case 2; [~,I] = sort(MY_REEFS.Reference_Area_km2(MY_REEFS.Reef_ID),'ascend'); % smallest area first  
        case 3; [~,I] = sort(MY_REEFS.GeomCH_3D_Area_km2(MY_REEFS.Reef_ID),'descend'); % largest area first
        case 4; [~,I] = sort(MY_REEFS.GeomCH_3D_Area_km2(MY_REEFS.Reef_ID),'ascend'); % smallest area first  
    end
    
    ORDERS(I,5) = MY_REEFS.Reef_ID;
end
...
[B,priority_list] = sortrows(ORDERS);
